...
namespace Typical_Tool {
	...
	namespace WindowsSystem {
		...
		// WindowKey
		enum TYPICALTOOL_API KeyTriggerMode { //按键触发模式 
			ClickOne, //单击一次: 按下并抬起
			Down, //按下: 
			Up, //抬起
		};
		...
		// 注册热键消息
		class TYPICALTOOL_API RegisterHotKeyMessage
		{
		private:
			Tstr Message;
		...
		//Shell消息
		class TYPICALTOOL_API ShellMessage {
		private:
			Tstr Message;
		...
		// 窗口托管
		class TYPICALTOOL_API WindowHost {
		public:
		...
		private:
			...
			static int32_t hMenu; //自动分配的菜单ID
		...
		// Shell配置
		class TYPICALTOOL_API ShellConfig {
		public:
		...
		// Shell执行
		class TYPICALTOOL_API WindowShell {
		private:
		...
		public:
			...
			* ShellArg: /k
			* I
			* Shell消息 temp("nvidia-smi", (int32_t)ShellExecute(NULL, "runas", "cmd", "nvidia-smi -lgc 1080", NULL, SW_SHOWNORMAL));
			*/
		...
		*/
		...
		TYPICALTOOL_API void SetDisplaySize(int32_t WidthValue, int32_t HeightValue, const Tstr& DisplayName = L"") {
		...
		*/
		template<class T = bool>
		TYPICALTOOL_API std::vector<Resolution> GetSupportedResolutions(__int32 _ResolutionIndex = ENUM_CURRENT_SETTINGS) {
		...
		template<class T = bool>
		TYPICALTOOL_API void WindowDPI() {
			// 设置 DPI 感知
		...
		//单一实例窗口程序
		template<class T = bool>
		TYPICALTOOL_API int32_t AloneRun(Tstr windowClassName, Tstr windowTitleName) {
			//程序启动初始化
		...
		//是否为管理员
		template<class T = bool>
		TYPICALTOOL_API bool IsUserAdmin() {
			BOOL retVal = FALSE;
		...
		//获得管理员权限
		template<class T = bool>
		TYPICALTOOL_API bool GainAdminPrivileges(Tstr strApp) {
		...
		*/
		template<class T = bool>
		TYPICALTOOL_API bool GetAdmin(bool isGet = true) {
			if (isGet) {
				//获取当前程序的全路径
		...
		// 添加注册表项以实现开机自启动
		template<class T = bool>
		TYPICALTOOL_API bool SetSelfStarting(Tstr valueName, Tstr exePath, Tstr CommandLineParam, bool _bAutoStart) {
			LONG result;
			HKEY hKey;
		...
		//提取程序目录名
		template<class T = bool>
		TYPICALTOOL_API bool ExtractExeDirectoryName(Tstr& path) {
		...
		//提取程序名
		template<class T = bool>
		TYPICALTOOL_API bool ExtractExeName(Tstr& path) {
			size_t lastSepPos = path.find_last_of(TEXT("\\/"));
			if (lastSepPos != Tstr::npos) {
		...
		//获取全路径程序名
		template<class T = bool>
		TYPICALTOOL_API bool GetExePathName(Tstr& _ExeName) {
