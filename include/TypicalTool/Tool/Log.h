...
namespace Typical_Tool
{
	...
	class TYPICALTOOL_API LogStringBuffer {
	public:
		Tstr AnsiEsc_Color;
		Tstr TimeStr;
		Tstr _Text;
	...
	enum class TYPICALTOOL_API LogMessage {
...
#define MB_ICONWARNING 0
	...
	*/
	...
	private:
		static void Log_Out(const Tstr& _ANSIESC_Color, Tostream& _Ostream, const Tstr& _str, const Tstr& _ANSIESC_RESET, const int32_t& _MessageKey = -1, bool _IsWirteFile = false) {
			if (!_Ostream.good()) {
				return;
			}
		...
		// _WINDOWS || _CONSOLE
		void Logs_ustr(const Tstr& _Text, const LogMessage& _Lm, const int32_t& _MessageKey) {
			switch (_Lm) {
	...
	public:
		...
		//显示/隐藏 Log消息
		void operator()(const Tstr& _Text, LogMessage(*_Lm)() = &Lnf, const int32_t& _MessageKey = -1)
		{
			if (!bShowLog) { return; }
		...
		void operator()(LogMessage(*_Lm)(), const Tstr& _Text, const int32_t& _MessageKey = -1)
		{
			if (!bShowLog) { return; }
	...
	public:
		...
		*/
		...
		void SetAllLogFileWrite(bool _LogFileWrite, const Tstr& _LogFileName, const Tstr& _LogFilePath, int32_t _LogLevel = static_cast<int32_t>(LogMessage::Error), bool bSingleLogFile = true)
		{
			...
			//输出所有级别
			if (_LogLevel != static_cast<int32_t>(LogMessage::Error)) {
				IsLogAllOutput = true;
	...
	* tip, war, Terr
	*/
	inline extern Log lg(false, false);
	/* 模式 mode: (_CONSOLE) && #ifdef _DEBUG
	* tip, war, Terr
	*/
	inline extern Log lgc(true, false);
	/* 模式 mode: (_CONSOLE | _WINDOWS) && #ifndef _DEBUG
	* tip, war, Terr
	*/
	inline extern Log lgr(false, true);
	/* 模式 mode: (_CONSOLE) && #ifndef _DEBUG
	* tip, war, Terr
	*/
	inline extern Log lgcr(true, true);

	inline extern Log& LogDebug = lgc;
	inline extern Log& LogRelease = lgcr;
}
